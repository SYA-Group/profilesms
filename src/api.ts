...
const API_URL = "https://profilesms.duckdns.org:5443/api";
...
// =====================================================
// Facebook Post Extraction Jobs
// =====================================================

export const createFacebookPostJob = async (data: {
  post_url: string;
  requested_count: number;
}) => {
  const res = await api.post("/facebook-post-jobs", data);
  return res.data;
};

export const listFacebookPostJobs = async () => {
  const res = await api.get("/facebook-post-jobs");
  return res.data;
};

export const getFacebookPostJobResults = async (jobId: number | string) => {
  const res = await api.get(`/facebook-post-jobs/${jobId}/results`);
  return res.data;
};
